...
def generate():
    try:
        ...
        filter = data.get('dietaryPreferences')
        if not ingredients:
            print("Missing ingredients")
            return jsonify({"message": "Missing ingredients"}), 400

        print(f"Generating recipe with ingredients: {ingredients} and filter: {filter}")
...
def add_recipe():
    ...
    recipe_id = database.add_recipe(user_id, name, ingredients, instructions)
    response = jsonify({"message": "Recipe added successfully", "recipe_id": recipe_id})
    return response, 200
